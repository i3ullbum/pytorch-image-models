...
#SBATCH --gres=gpu:4                   # Request 8 GPUs (adapt if you only have 2 or 4 per node)
#SBATCH --nodes=2                      # Number of nodes (1 node here; if you need multiple nodes, increase this)
...
module purge
module load nvtop/1.1.0 htop/3.0.5 singularity/4.1.0
